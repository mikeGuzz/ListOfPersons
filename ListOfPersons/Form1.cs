...
namespace ListOfPersons
{
    public partial class Form1 : Form
    {
        ...
        private void edit_button_Click(object sender, EventArgs e)
        {
            ...
            if (listBox.SelectedItem is Person ob)
            {
                var dialog = new ItemForm("Edit item", ob);
                dialog.ShowDialog();
                listBox.Items[index] = dialog.GetPerson();
            }
            else
            {
        ...
        {
            foreach (Person ob in listBox.SelectedItems)
            {
                ob.Preview();
            }
        }
        ...
        {
            Person[] buffer = new Person[listBox.Items.Count];
